...
// Article fields are author-supplied and go through innerHTML below, so
// anything interpolated has to be escaped first.
function esc(s) {
  return String(s ?? '').replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
}

// Only http(s) and relative URLs may reach an href/src, so a javascript: or
// data: payload in image_url or a markdown link can't execute.
...
async function shareArticle(article) {
  ...
  if (navigator.share) {
    try {
      await navigator.share({ title: article.title || 'Minecraft Club of America', url });
      return;
        ...
        <div class="article-hero">
          <div class="article-hero-inner">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.5rem;gap:1rem;flex-wrap:wrap;">
              ...
              <button id="share-btn" type="button" aria-label="Copy a link to this article" style="display:inline-flex;align-items:center;gap:8px;padding:0.45rem 1.1rem;border-radius:6px;border:1px solid var(--border);background:var(--white);color:var(--black);font-size:13px;font-weight:500;cursor:pointer;transition:background .15s,color .15s;">
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>
                Share
              </button>
            </div>
            ${safeUrl(article.image_url) ? `<div class="article-image" style="margin-bottom:2rem;"><img src="${esc(safeUrl(article.image_url))}" alt="${esc(article.title)}"></div>` : ''}
