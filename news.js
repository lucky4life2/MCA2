// news.js — self-contained, no top-level import/export
// Loaded via plain <script src="news.js"> in news.html
...
(async function () {
  ...
  // ── Featured banner (category === 'featured') ─────────────────
  // Stored in sessionStorage so closing persists for the session.
  function renderFeaturedBanner(article) {
    ...
    // Insert above .page-wrap or at top of body
  ...
  function applySearch(query) {
    ...
    const count = document.getElementById('news-search-count');
    ...
    if (count) count.textContent = q ? `${filtered.length} result${filtered.length !== 1 ? 's' : ''}` : '';
    ...
    if (filtered.length === 0) {
      ...
      index.innerHTML = filtered.map(renderCard).join('');
      index.querySelectorAll('.news-card').forEach((el, i) => {
  ...
  if (!indexEl) return; // not on news page
  ...
  try {
    ...
    // Show featured banner for the first article with category "featured"
    ...
    // Store all articles and render list
